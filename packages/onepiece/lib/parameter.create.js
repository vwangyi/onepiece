...
import { inquirerConfirm } from './utils.js';
import { inquirerChoose } from './utils.js';
import { inquirerInputs } from './utils.js';
  ...
  program
    ...
    .action(async (name, option) => {
      if (!shell.which('git')) {
        console.log(
          logSymbols.error,
          chalk.redBright('对不起,运行脚本必须先安装git')
        );
        shell.exit(1); // 退出当前进程
      }
      // 验证name输入是否符合规范
      ...
      // 验证 onepiece create xxxxx -t xxx
      let repository = '';
      if (option.template) {
