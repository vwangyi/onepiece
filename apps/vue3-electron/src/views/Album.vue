...
console.log('Amplitude', Amplitude.getConfig());
...
// 这里需要定义几个状态
const albumInfo = ref([]); //  存储专辑信息
const albumSongs = ref([]); //  存储专辑中的歌曲信息
const selectdAlbum = ref(null); //  存储当前选中的专辑
...
onMounted(() => {
  ...
  // 操作完成后，我们再重新将其转为数组
  albumInfo.value = Array.from(uniqueAlbums.values());
});
